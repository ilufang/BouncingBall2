int cx = 800, cy = 600;
Slider slider = new Slider(-5, 5, 0.2, 0, 10, 10, int 100, int 25, #ccccff, #66ccff);
...
class Slider extends Control
{
	...
	Slider(float min_val, float max_val, float unit, float init_val, int x_pos, int y_pos, int width, int height, color background, color tint)
	{
		...
		step = unit;
		value = init_val;
		slider_w = step/(max-min)*width+step;
		slider_x = (value-min)/(max-min);
	}
	void draw()
	{
		super.draw();
